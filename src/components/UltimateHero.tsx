...
import { FaGraduationCap, FaBolt, FaShieldAlt, FaCheckCircle } from "react-icons/fa";
...
  const floatY = useTransform(mouseY, (v) => `${v * 12}px`);

  // Animation variants
  const fadeIn = { hidden: { opacity: 0, y: 18 }, show: { opacity: 1, y: 0 } };
  // Stagger delay set to 0.1
...
      ref={containerRef}
      aria-label="Hero — Grad Manager"
      // FIX 1: Reduced minimum height on mobile
...
          <motion.div 
            // Removed variants={fadeIn} from this outer div so the children can be the staggered items
            className="lg:col-span-7 text-center lg:text-left" 
          >
            {/* 1. Badge */}
            <motion.div variants={fadeIn} className="inline-flex items-center gap-3 px-3 py-1 rounded-full bg-white backdrop-blur-md border border-gray-200 shadow-md">
              <div className="w-2 h-2 rounded-full bg-gradient-to-r from-blue-500 to-pink-400 shadow-sm" />
              <span className="text-xs font-semibold text-slate-700">New • Smart Matches</span>
            </motion.div>

            {/* 2. Title */}
            <motion.h1 variants={fadeIn} className="mt-6 text-4xl sm:text-5xl md:text-6xl font-extrabold tracking-tight text-gray-900 leading-tight">
              <span className="block">{heroTitle}</span>
              <span
                className="block mt-2 text-transparent bg-clip-text bg-gradient-to-r from-blue-500 via-purple-500 to-indigo-600"
              >
                {heroSubTitle}
              </span>
            </motion.h1>
            ...
            <DynamicTextHero />
            {/* ⭐️ END DYNAMIC TEXT HERO ⭐️ */}


            {/* 3. Body Text */}
            <motion.p variants={fadeIn} className="mt-5 text-lg text-gray-700 max-w-2xl mx-auto lg:mx-0">{heroText}</motion.p>

            {/* 4. CTA Button */}
            <motion.div variants={fadeIn} className="mt-8 flex flex-col sm:flex-row items-center gap-4 justify-center lg:justify-start">
              <Link
                to="/signup"
...
                <span className="uppercase tracking-wide">Join Grad Manager</span>
              </Link>
            </motion.div>
...
            >
              <FeatureCard icon={<FaBolt className="text-blue-600" />} title="Curated funded programs" subtitle="" mouseX={mouseX} mouseY={mouseY} />
              <FeatureCard icon={<FaShieldAlt className="text-pink-600" />} title="Verified info" subtitle="" mouseX={mouseX} mouseY={mouseY} />
              <FeatureCard icon={<FaCheckCircle className="text-teal-600" />} title="Mentors & reviews" subtitle="" mouseX={mouseX} mouseY={mouseY} />
            </motion.div>
          </motion.div>
...
                </div>
                <div>
                  <div className="text-sm font-semibold text-slate-900">Top-rated guidance</div>
                  <div className="text-xs text-slate-600">Students rated 4.8/5 on outcome & service</div>
                </div>
              </motion.div>
            </motion.div>
          </motion.div>
...
  const ty = useTransform(mouseY, (v: number) => `${v * 6}px`);
  
  // Define a custom variant for feature cards if you want them to stagger internally (e.g., scale up)
  const cardFadeIn = { hidden: { opacity: 0, scale: 0.9 }, show: { opacity: 1, scale: 1 } };
  
  return (
    <motion.div
